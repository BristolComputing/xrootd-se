...
ARG XROOTD_GID=1094
ARG XROOTD_UID=1094
ARG XROOTD_VERSION="5.6.9-1.el9"
...
RUN curl -LO "https://repo.anaconda.com/miniconda/Miniconda3-latest-Linux-x86_64.sh" \
  ...
  && conda init \
  && conda install -y -q python=3.11 \
  && conda clean --all
ENV PATH=/miniconda/bin:$PATH
ARG XRDSUM_VERSION=2024.6.1
RUN /miniconda/bin/pip --no-cache-dir install xrdsum[posix]==${XRDSUM_VERSION}

# gather info and test gathering script
RUN mkdir -p /etc/xrootd_info
